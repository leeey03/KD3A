...
from train.train_fedavg_baseline import train, test
...
def main_fedavg(args=args, configs=configs):
    ...
    print("Begin the {} time's training, Dataset:{}, Source Domains {}, Target Domain {}".format(args.train_time,
                                                                                                 ...
                                                                                                 args.source_domains,
                                                                                                 args.target_domain))
    # initialize domain weight
        # [0]: global model (target domain), [1:-1]: source domains
    domain_weight = [0] + [round(1 / (len(args.source_domains)), 4) for i in range(len(args.source_domains))] # equal weights to all sources
    ...
    for epoch in range(args.start_epoch, total_epochs):
        ...
        train(train_dloaders, models, classifiers, optimizers, classifier_optimizers, 
              epoch, writer, num_classes=num_classes, domain_weight=domain_weight,
              source_domains=args.source_domains, batch_per_epoch=batch_per_epoch, 

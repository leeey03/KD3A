...
from train.train_multiscale_late import train, test
from model.multiscale import PerScaleTemporalTransformer, ClassificationHead, FusionHead
...
def main(args=args, configs=configs):
    ...
    if configs["DataConfig"]["dataset"] == "EpicKitchens":
        ...
        classifiers.append(classifier_set)
    ...
    for model in models[1:]:
        for scale in scale_names:
            for source_weight, target_weight in zip(model[scale].named_parameters(), models[0][scale].named_parameters()):
    ...
    for model in models:
        optimizer_set = {}
        for scale in scale_names:
            optimizer_set[scale] = torch.optim.SGD(model[scale].parameters(), momentum=args.momentum,
                                                   lr=configs["TrainingConfig"]["learning_rate_begin"], weight_decay=args.wd)
        optimizers.append(optimizer_set)
    for classifier in classifiers:
        classifier_optimizers_set = {}
        for scale in scale_names + ['fusion']:
            # print(f"Classifier {classifier.name} device:", next(classifier.parameters()).device)
            classifier_optimizers_set[scale] = torch.optim.SGD(classifier[scale].parameters(), momentum=args.momentum,
                                                               lr=configs["TrainingConfig"]["learning_rate_begin"],
                                                               weight_decay=args.wd)
        classifier_optimizers.append(classifier_optimizers_set)
    # create the optimizer scheduler with cosine annealing schedule
    for optimizer in optimizers:
        optimizer_schedulers_set = {}
        for scale in scale_names:
            optimizer_schedulers_set[scale] = CosineAnnealingLR(optimizer[scale],
                                                                configs["TrainingConfig"]["total_epochs"],
                                                                eta_min=configs["TrainingConfig"][
                                                                    "learning_rate_end"])
        optimizer_schedulers.append(optimizer_schedulers_set)
    for classifier_optimizer in classifier_optimizers:
        classifier_optimizer_schedulers_set = {}
        for scale in scale_names + ['fusion']:
            classifier_optimizer_schedulers_set[scale] = CosineAnnealingLR(classifier_optimizer[scale],
                                                                          configs["TrainingConfig"]["total_epochs"],
                                                                          eta_min=configs["TrainingConfig"]["learning_rate_end"])
        classifier_optimizer_schedulers.append(classifier_optimizer_schedulers_set)
    ...
    for epoch in range(args.start_epoch, total_epochs):
        ...
        domain_weight = train(train_dloaders, models, classifiers, optimizers, classifier_optimizers,
